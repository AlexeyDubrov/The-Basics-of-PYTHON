...
class Data:
    ...
    @classmethod
    def extract(cls):
        str_m = list(map(int, cls.str_data.split('-')))
        return str_m

    @staticmethod
    def perm():
        list_m = Data.extract()
        if (0 < list_m[0] < 30) and (0 < list_m[1] < 13) and (0 < list_m[2] < 1000):
            val = f"Data {Data.str_data} correct"
        else:
            val = f"Data {Data.str_data} incorrect"
        return val


data_1 = Data("12-42-20")
...
class ComplexNumber:
    ...
    def __add__(self, other):
        return f"Summ complex numbers {self.num_comp + other.num_comp}"

    def __mul__(self, other):
        return f"Multiplication of complex numbers {self.num_comp * other.num_comp}"
